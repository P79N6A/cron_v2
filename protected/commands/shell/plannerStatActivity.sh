...
${yiiPath} planner statActivity --stat_date=$stat_date --is_stat_score=1 >> ${logPath}/$(basename $0 .sh)_${curDate}.log 2>&1
result=$?;
if [ $result -ne 0 ]
then
    cur_time=`date "+%Y-%m-%d %H:%M:%S"`;
    echo "${yiiPath} planner statActivity --stat_date=$stat_date --is_stat_score=1" >> ${logPath}/$(basename $0 .sh)_${curDate}.log
