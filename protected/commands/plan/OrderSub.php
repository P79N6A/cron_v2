...
class  OrderSub{
	...
	public function OrderSub(){
		...
		try{
			...
			$wsclient->connect(PlanOrder::model()->cuohe_host ,PlanOrder::model()->cuohe_port ,'/sub',base64_encode(PlanOrder::model()->cuohe_user .":".PlanOrder::model()->cuohe_passwd ));
			$wsclient->setCallBack(array('OrderSub','ReceiveData'));
			...
			if ($pid==0) {//子进程
                if(defined('ENV') && ENV == 'dev'){
                    ...
                    $redis_key = 'lcs_plan_cuohe_starttime';
                    //撮合系统的状态
                    $sys_arr = Yii::app()->curl->get("http://".PlanOrder::model()->cuohe_host.":".PlanOrder::model()->cuohe_port."/status");
				...
				if(!empty($val)){
					...
					if($arr['project'] == 'licaishi' && $arr['type']=='cn'){
						...
						if(defined('ENV') && ENV == 'dev'){//测试环境
							exec("php  ~/cron/cron_v2/protected/yiic.php Plan DealCuohe --type=$type --pln_id=$pln_id --order_id=$order_id --symbol=$symbol --price=$price --volume=$volume --time=$time --seqid=$seqid --is_back=0");
						}else{
							exec("php  ~/projects/cron/licaishi/protected/yiic.php Plan DealCuohe --type=$type --pln_id=$pln_id --order_id=$order_id --symbol=$symbol --price=$price --volume=$volume --time=$time --seqid=$seqid --is_back=0");
						}
